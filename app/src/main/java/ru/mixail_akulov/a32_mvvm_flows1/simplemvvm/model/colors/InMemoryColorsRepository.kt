...
class InMemoryColorsRepository(
    ...
    override fun listenCurrentColor(): Flow<NamedColor> = callbackFlow {
        ...
        awaitClose {
            // этот блок выполняется при отмене закрытия, полезен для логики очистки
            listeners.remove(listener)
        }
    }.buffer(Channel.CONFLATED) // Channel.CONFLATED полезно, когда вам нужны только самые последние значения
